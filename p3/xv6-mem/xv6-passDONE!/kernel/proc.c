...
userinit(void)
{
  ...
  acquire(&ptable.lock);
  initproc = p;
  //sx: kvm kernal part, nothing related to user
...
growproc(int n)
{
  ...
  proc->sz_heap = sz;
  //cprintf("-growproc end: sz_heap(%d)\n-switchuvm\n",sz);
  switchuvm(proc);
  return 0;
}
...
fork(void)
{
  ...
  if((np->pgdir = copyuvm(proc->pgdir, proc->sz_heap)) == 0){
    kfree(np->kstack);
    np->kstack = 0;
    np->state = UNUSED;
    return -1;
