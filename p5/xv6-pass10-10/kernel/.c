...
stati(struct inode *ip, struct stat *st)
{
  ...
  if(ip->addrs[0]==0)
	//panic("ip->addrs[0]==0");
  st->checksum = ip->addrs[0];
  for(i=1;ip->addrs[i]!=0;i++){
  	st->checksum ^= ip->addrs[i];
  }
}

uchar
fetchChecksum(struct inode *ip, uint nb)
{
	uchar checksum = 0x00;
	return ((ip->addrs[nb]>>24)|checksum);
}

int
checkSum(struct inode *ip, char *dst,uint nb, uint m)
{
	if(compChecksumb(dst,nb,m) != fetchChecksum(ip,nb))
		return -1;
	return 0;
}
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    if(ip->type == T_CHECKED){
			cprintf("readi:checksum start\n");
			//if(checkSum(ip,dst,off%BSIZE,m)==-1)
			return -1;
		}
...
//block size: 512Byte
uchar
compChecksumb(char *addr, uint nthblock, uint m)
{
	uchar checksum = 0;
	int bytei=0;
	checksum = addr[nthblock+0];
	for(bytei=1;bytei<m;bytei++)
	{
		checksum ^= addr[nthblock+bytei];
	}
	return checksum;
}

int
updateChecksum(struct inode *ip, uint nb, char *src, uint m)
{
	uint checksumNoffset = 0x00000000;
	ip->addrs[nb]	&=0x00FFFFFF;
	ip->addrs[nb]	|=((compChecksumb(src, nb, m)|checksumNoffset)<<24);
	return 0;
}
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    //sx:write checksum into inode
    if(ip->type == T_CHECKED){
			cprintf("writei:T_CHECKED start updateChecksum\n");
    	updateChecksum(ip,off/BSIZE,src,m);
		}
    memmove(bp->data + off%BSIZE, src, m);
    bwrite(bp);
    brelse(bp);
  }
	cprintf("writei:sth(%s)\n",src);
