...
{
	clr_enter(in_buf);
	if(is_pipe()==1) return 1; //nothing return 1
  ...
  //	if(*cmd_buf!='\0'&&*cmd_buf!='\r'&&*cmd_buf!='\n'&&cmd_buf!= NULL)
	...
	else return 1; //nothing in, return 1
  return 0;
}
...
int is_pipe()
{
	...
	CmdPipe.flag=0;	//0->no cmd,1->pipe,<1->error
	int i =0;
  	if(*in_buf!='\0'&&*in_buf!=10)
 	{
		...
		while(1) 
		{
			...
			{
				if(CmdPipe.flag>1) unexp_error(0); //store the number of the cmd	
				break;	//detected all >
			}
	...
	else	return 1; //nothing return 1
	printf("CmdIn:%s(%d)\nPipeBuf:%s\n",cmd_buf,cmd_buf[0],pipe_buffer[0]);
	return 0;
}

int is_python() //is python return 1, not return 0
{
	if(strstr(CmdIn.argv[0],".py")!=NULL) return 1;
	return 0;
}
