...
scheduler(void)
{
  ...
  for(;;){
    ...
    sti();
	//sx==================
    hcounter = 0;//don NOT move this line
	lcounter = 0;
	//===================
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;
         ...
         int tlTicks=get_totalTicks(1);
	//sx:lot sched=====================================
		 if(ck_queue == 0){ //high queue
        	winner = get_rand()%thTicks;    //winner =random
         	hcounter += p->ticks.hticks;
         	if(hcounter < winner) continue;     //continue for loop 
			p->ticks.HTimes ++; 
			//cprintf("pid(%d)htickets(%d)ltickets(%d)queue(%d)Htimes(%d)Ltimes(%d)\n",p->pid,p->ticks.hticks,p->ticks.lticks,p->ticks.queue,p->ticks.HTimes,p->ticks.LTimes); 
			p->ticks.queue = 1; //move to low 
			p->ticks.lticks = p->ticks.hticks;
			}
		 else{
			winner = get_rand()%tlTicks;
			lcounter += p->ticks.lticks;
			if(lcounter < winner) continue;
			p->ticks.LTimes ++;
         	//cprintf("pid(%d)htickets(%d)ltickets(%d)queue(%d)Htimes(%d)Ltimes(%d)\n",p->pid,p->ticks.hticks,p->ticks.lticks,p->ticks.queue,p->ticks.HTimes,p->ticks.LTimes);	
			conti_flag = 1;
...
int getpinfo(struct pstat * p_pstat)
{
	...
	for(i=0;i<NPROC;i++){
		...
		p_pstat->tickets[i] = 	ptable.proc[i].ticks.hticks;
	}
	return 0;
}
//sx settickets
int settickets(int tickets)
{
	...
	for(i =0 ;i<NPROC;i++){
		...
		ptable.proc[i].ticks.hticks =tickets;
		ptable.proc[i].ticks.lticks =tickets;
		}
	}
	//release(&ptable.lock);
	return 0;
